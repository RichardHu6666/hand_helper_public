...
static void cleanup_failed_sensor_init(i2c_master_bus_handle_t i2c_bus_handle,
                                       esp_sccb_io_handle_t sccb_handle)
{
    if (sccb_handle) {
        esp_err_t ret = esp_sccb_del_i2c_io(sccb_handle);
        if (ret != ESP_OK) {
            ESP_LOGW(TAG, "Failed to delete SCCB IO during cleanup: %s", esp_err_to_name(ret));
        }
    }
    if (i2c_bus_handle) {
        esp_err_t ret = i2c_del_master_bus(i2c_bus_handle);
        if (ret != ESP_OK) {
            ESP_LOGW(TAG, "Failed to delete I2C bus during cleanup: %s", esp_err_to_name(ret));
        }
    }
}
...
void example_sensor_init(example_sensor_config_t *sensor_config, example_sensor_handle_t *out_sensor_handle)
{
    ...
    if (!sensor_config || !out_sensor_handle) {
        ESP_LOGE(TAG, "invalid sensor init arguments");
        return;
    }
    memset(out_sensor_handle, 0, sizeof(*out_sensor_handle));
    ...
    for (esp_cam_sensor_detect_fn_t *p = &__esp_cam_sensor_detect_fn_array_start; p < &__esp_cam_sensor_detect_fn_array_end; ++p) {
        ...
        if (cam) {
            if (p->port != sensor_config->port) {
                ...
                cleanup_failed_sensor_init(i2c_bus_handle, cam_config.sccb_handle);
                return;
            }
            break;
        }
    ...
    if (!cam) {
        ...
        cleanup_failed_sensor_init(i2c_bus_handle, NULL);
        return;
    }
    ...
    out_sensor_handle->sensor_dev = cam;
}
